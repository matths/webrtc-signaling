/*
based on https://github.com/bcamarneiro/macfromip
but replace an arp -a with a single ip resolution which is much faster
*/
'use strict';
...
//      var ls2 = cp.exec('arp -a',
      var ls2 = cp.exec('arp ' + ipAddress,
        function(error2, stdout2, stderr2) {
          ...
          stdout2 = stdout2.replace(/.*(^([0-9A-Fa-f]{1,2}[:-]){5}([0-9A-Fa-f]{1,2})).*/, '$1');
          stdout2 = stdout2.split(':').map(function (item) { return (item.length==1?'0':'')+item; }).join(':');
